...
async function initSubmodules(skipPrompt = false) {
  const allProjects: Project[] = Object.entries(vendors).map(([name, config]) => ({
    name,
    url: config.source,
    path: `vendor/${name}`,
  }))
  ...
  // Check for extra submodules not in meta.ts
  ...
  const selected = skipPrompt
    ? newProjects
    : await p.multiselect({
        message: 'Select vendors to initialize',
...
async function syncSubmodules() {
  ...
  for (const [vendorName, config] of Object.entries(vendors)) {
    ...
    for (const [sourceSkillName, outputSkillName] of Object.entries(config.skills)) {
      ...
      spinner.start(`Syncing: ${sourceSkillName} → ${outputSkillName}`)
      ...
      writeFileSync(join(outputPath, 'SYNC.md'), `# Sync Info
...
async function checkUpdates() {
  ...
  const updates: { name: string, behind: number, skills: string }[] = []
  ...
  for (const [name, config] of Object.entries(vendors)) {
    ...
    if (count > 0) {
      ...
      updates.push({ name, behind: count, skills: skillNames })
    }
  }

  if (updates.length === 0) {
    p.log.success('All vendors are up to date')
  }
  else {
    p.log.info('Updates available:')
    for (const update of updates) {
      p.log.message(`  ${update.name}: ${update.behind} commits behind (${update.skills})`)
    }
  }
}
...
async function main() {
  ...
  const action = await p.select({
    ...
    options: [
      { value: 'sync', label: 'Sync vendor skills', hint: 'Update submodules and copy skills' },
      { value: 'init', label: 'Init submodules', hint: 'Add new vendors as submodules' },
      { value: 'check', label: 'Check updates', hint: 'See available upstream updates' },
